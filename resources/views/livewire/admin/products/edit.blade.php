<div>
    <div class="flex flex-wrap items-start justify-between gap-4 mb-6">
        <div>
            ...
            <flux:text class="mt-1 text-zinc-500">Update "{{ $product->name }}".</flux:text>
        </div>
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="rounded-xl border border-zinc-200 dark:border-zinc-700 p-6 space-y-6">
                    ...
                    <flux:field>
                        ...
                        <flux:description>Regenerated automatically if you change the product name, and kept unique. This can't be edited directly.</flux:description>
            ...
            <div class="space-y-6">
                ...
                <div class="rounded-xl border border-zinc-200 dark:border-zinc-700 p-6 space-y-4">
                    ...
                    @if (count($existingImages))
                        <div>
                            <flux:text size="sm" class="text-zinc-500 mb-2 block">Current Images</flux:text>

                            <div class="space-y-3">
                                @foreach ($existingImages as $index => $image)
                                    <div wire:key="existing-image-{{ $image['id'] }}" class="flex items-center gap-3 rounded-lg border border-zinc-200 dark:border-zinc-700 p-2">
                                        <img src="{{ asset('storage/' . $image['path']) }}" class="h-14 w-14 rounded-md object-cover shrink-0" />

                                        <div class="flex-1 min-w-0">
                                            <button
                                                type="button"
                                                wire:click="setPrimaryImage('existing-{{ $image['id'] }}')"
                                                class="text-xs {{ $primaryImageKey === 'existing-' . $image['id'] ? 'text-blue-600 font-medium' : 'text-zinc-500 hover:text-zinc-700' }}"
                                            >
                                                {{ $primaryImageKey === 'existing-' . $image['id'] ? '★ Primary image' : 'Set as primary' }}
                                            </button>
                                        </div>
                                        ...
                                        <div class="flex flex-col gap-0.5">
                                            <flux:button
                                                ...
                                                wire:click="moveExistingImageUp({{ $index }})"
                                                :disabled="$index === 0"
                                            />
                                            <flux:button
                                                size="sm" variant="ghost" icon="chevron-down"
                                                wire:click="moveExistingImageDown({{ $index }})"
                                                :disabled="$index === count($existingImages) - 1"
                                            />
                                        </div>
                    ...
                    <div>
                        ...
                        <div wire:loading wire:target="newImages" class="text-sm text-zinc-500 mt-2">
                            Uploading...
                        </div>
                        ...
                        @if (count($newImages))
                            <div class="space-y-3 mt-3">
                                @foreach ($newImages as $index => $image)
                                    <div wire:key="new-image-{{ $index }}" class="flex items-center gap-3 rounded-lg border border-zinc-200 dark:border-zinc-700 p-2">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            ...
                                            @else
                                                <button
                                                    type="button"
                                                    wire:click="setPrimaryImage('new-{{ $index }}')"
                                                    class="text-xs mt-1 {{ $primaryImageKey === 'new-' . $index ? 'text-blue-600 font-medium' : 'text-zinc-500 hover:text-zinc-700' }}"
                                                >
                                                    {{ $primaryImageKey === 'new-' . $index ? '★ Primary image' : 'Set as primary' }}
                                                </button>
                                            @enderror
                                        </div>
                                        ...
                                        <div class="flex flex-col gap-0.5">
                                            <flux:button
                                                size="sm" variant="ghost" icon="chevron-up"
                                                wire:click="moveNewImageUp({{ $index }})"
                                                :disabled="$index === 0"
                                            />
                                            <flux:button
                                                size="sm" variant="ghost" icon="chevron-down"
                                                wire:click="moveNewImageDown({{ $index }})"
                                                :disabled="$index === count($newImages) - 1"
                                            />
                                        </div>
                    ...
                    @if (! count($existingImages) && ! count($newImages))
